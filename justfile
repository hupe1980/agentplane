...
test-mcp:
    cargo test --features mcp,sqlite,testkit --test wire mcp::

# the operator surface, with only its own features on
test-http:
    cargo test --features http,sqlite --test wire api::

# signed journals, including a wholesale rewrite
test-attestation:
    cargo test --features signing,sqlite --test trust attestation::

# A2A and model-driver failure mappings
test-drivers:
    cargo test --features a2a,providers,http,sqlite --test wire drivers::
