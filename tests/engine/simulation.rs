...
fn runtime(store: &Arc<SqliteStore>, world: &World, failing: Option<&'static str>) -> Runtime {
...
async fn crash_at(records: &[Record], n: usize, run: RunId) -> Arc<SqliteStore> {
...
async fn sweep(failing: Option<&'static str>) {
    let origin = Arc::new(SqliteStore::open_in_memory().unwrap());
