...
| ⬜ | **The rest of format freeze** — canonicalization is versioned now, so a rule change reads as *unverifiable* rather than as a divergence. What remains under this heading is the rest of the ceremony: golden vectors from another implementation where practical, an unknown-field policy, migration and rollback procedures, and an algorithm-agility plan for every durable or signed format |
