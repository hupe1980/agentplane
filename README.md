...
| 🔑 | **Erasure that reaches the backups** — deleting clears the live store; the backup taken an hour earlier still has everything, and backups are offsite and often immutable *by design*. So payload bytes are sealed under a per-case data key wrapped by a key the crate never holds: erasing a case **destroys the key**, and every copy becomes unreadable at once — including the ones nobody can reach. Rotation re-wraps without rewriting bulk data; an erased case never comes back. `VaultTransit` speaks Vault's transit engine, so the wrapping key never leaves Vault. **Scope matters and is documented rather than implied**: this covers blob payloads and memory content. Journal rows are plaintext and append-only, so prompts and tool arguments — which are effect identity — are permanent; erasable data goes in a blob and the chain commits to the digest → [erasure and keys](https://hupe1980.github.io/agentplane/docs/erasure/) |
