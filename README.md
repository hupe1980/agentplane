...
| 📡 | **An honest A2A 1.0 server subset** — blocking/non-blocking tasks, reconstructable `GetTask`, journal-backed status/artifact streaming, cursor-paginated `ListTasks` with optional artifacts, context-based multi-turn as new immutable runs in one case, cancellation and extended cards. `taskId` mutation remains refused: continue with `contextId`. Parts and roles are validated, remote/raw media is refused, every operation is authorized, and push is explicitly unsupported until a transactional outbox can provide durable delivery |
...
| 🔔 | **A governed webhook transport, not fake A2A push** — `PushSender` permits only operator-granted HTTPS hosts after all-address checks and DNS pinning, and sends state rather than output. The A2A card still advertises push false: transport controls and registration storage are insufficient without a transactional delivery outbox and worker |
