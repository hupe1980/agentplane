...
| 📡 | **An honest A2A 1.0 server subset** — the public card declares deployment-wired bearer auth and never advertises push until its store/sender exist. Blocking calls return completed tasks with the actual text/data artifact; non-blocking calls return working tasks. Parts are oneof-validated, remote/raw media is refused, every operation is authorized, and peer input is untrusted. `ListTasks` and multi-turn continuation say unsupported rather than silently doing something else |
...
| 🧠 | **Memory that cannot promote itself** — private-agent and shared-team memory use subject/purpose scopes on redb or PostgreSQL. Runtime writes derive labels from `Tainted<Value>`, removing metadata laundering. Retrieval journals exact versions and security commitments. Immutable expiry is evaluated against journaled time; legal hold blocks every erasure path; expiry sweeping is backend-atomic. Semantic ranking is a separate journaled `SemanticRetriever` with embedding revision, index snapshot, scores and exact selected commitments — the vector index never becomes memory truth. Memory cryptographic deletion from backups remains open |
