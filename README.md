...
| 🔌 | **Real tool and peer wires** — a genuine MCP *client* (not "your tools, MCP-shaped"), several servers at once beside typed in-process tools, and an A2A 1.0 JSON-RPC client and server, each mapping wire outcomes conservatively into recovery |
...
| 📡 | **An honest A2A 1.0 server subset** — blocking/non-blocking tasks, reconstructable `GetTask`, journal-backed status/artifact streaming, cursor-paginated `ListTasks` with optional artifacts, context-based multi-turn as new immutable runs in one case, cancellation, durable deployment-gated push and extended cards. `taskId` continuation is not implemented and is refused; this means an `INPUT_REQUIRED` task cannot yet resume through standard A2A multi-turn. Parts and roles are validated, remote/raw media is refused, and every operation is authorized |
