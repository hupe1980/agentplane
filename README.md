...
| 🧠 | **Memory that cannot promote itself** — private-agent and shared-team memory use policy-authorized subject/purpose scopes on redb or PostgreSQL. Runtime writes take `MemoryWrite` plus `Tainted<Value>` and derive labels, removing the metadata laundering path. Retrieval journals exact versions and security commitments. Revisions and cascading graph erasure are backend-atomic; erased ids never recycle. Run history, case state, durable memory and one-call model context remain separate; provider conversations and opaque compaction are not replay truth |
