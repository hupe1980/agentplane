...
| 📡 | **An honest A2A 1.0 server subset** — blocking/non-blocking tasks, reconstructable `GetTask`, journal-backed status/artifact streaming, cursor-paginated `ListTasks` with optional artifacts, context-based multi-turn as new immutable runs in one case, cancellation, durable deployment-gated push and extended cards. `taskId` mutation remains refused: continue with `contextId`. Parts and roles are validated, remote/raw media is refused, and every operation is authorized |
