...
cargo run --example tool_loop          # a model choosing tools, and four refusals
...
| 🧠 | **Memory that cannot promote itself** — a recalled memory is labelled by *where it came from*, never by what it says, because text asserting its own reliability is the cheapest thing an attacker can write. Retrieval is a journaled effect, so a replayed run reads what it read rather than what a later search would find; writes supersede rather than edit, so forgetting is selective and reaches every version. Summarising is an effect with the same rules: a summary inherits the join of what it summarised rather than declaring its own label, is bounded by a ceiling on what the summarising model may be shown, and records the versions it absorbed so forgetting can reach them |
...
| 🧰 | **A tool is one type** — arguments are fields, the schema comes from the fields, the description from the doc comment, and `call` takes `self` so the body receives the declared shape or the call was refused. No `Value` to index, no field to misspell, no dispatch on a name. And `.toolbox(..)` derives the catalogue from the agent's own declaration and refuses to build where the code and the reviewed manifest have drifted either way |
