...
| 🧠 | **Memory that cannot promote itself** — private-agent and shared-team memory use policy-authorized subject/purpose scopes on redb or PostgreSQL. A recalled memory is labelled by *where it came from*, never by what it says. Retrieval is a journaled effect, so replay reads exact selected versions and security metadata rather than re-searching a moved corpus or accepting identical bytes under a promoted label. Writes serialize, supersede rather than edit, and never recycle an erased id. A correction preserves lineage so a later cascading erasure can still reach summaries. Run history, case/conversation state, durable memory and one-call model context remain separate; provider conversations and opaque compaction are not replay truth |
