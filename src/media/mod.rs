...
fn validate_resolved(
    ...
    for address in &addresses {
        if !is_public_ip(address.ip()) {
            return Err(MediaError::Refused(format!(
                "media host '{host}' resolved to forbidden address {}",
                address.ip()
            )));
        }
    }
...
mod tests {
    ...
    #[test]
    fn every_private_metadata_and_special_address_family_is_refused() {
        for ip in [
            "0.0.0.0",
            "10.0.0.1",
            "100.64.0.1",
            ...
            assert!(!is_public_ip(ip.parse().unwrap()), "{ip} was accepted");
        }
        for ip in ["1.1.1.1", "8.8.8.8", "2606:4700:4700::1111"] {
            assert!(is_public_ip(ip.parse().unwrap()), "{ip} was refused");
        }
    }
    ...
    fn one_private_dns_answer_refuses_the_entire_resolution() {
        ...
        assert!(
            result.is_err(),
            "a public answer must not launder a private one"
        );
    }
